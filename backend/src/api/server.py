...
# force=True overrides handlers set by Azure Monitor/telemetry
logging.basicConfig(
    level=logging.INFO,
...
# Silence ONLY the noisy third-party loggers
for noisy_logger in [
    "httpx", "httpcore", "urllib3", "requests",
    "azure", "opentelemetry", "azure.monitor",
...
# ========== STEP 5c: AUTO-INDEX ON STARTUP ==========
@app.on_event("startup")
async def auto_index_check():
    """Checks if the knowledge base has documents. If empty, indexes PDFs automatically."""
    try:
        ...
        if len(results) == 0:
            logger.info("📚 Index is empty — auto-indexing compliance PDFs...")
            from backend.scripts.index_documents import index_docs
            await asyncio.to_thread(index_docs)
            logger.info("✅ Auto-indexing complete! Knowledge base is ready.")
        else:
            logger.info("✅ Knowledge base already has documents — skipping auto-index.")
    except Exception as e:
        logger.warning(f"⚠️ Could not check index: {e}. Run manually: uv run python -m backend.scripts.index_documents")
...
async def audit_video(request: AuditRequest):
    ...
    try:
        ...
        final_state = await asyncio.to_thread(compliance_graph.invoke, initial_inputs)
